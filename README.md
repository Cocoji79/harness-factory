...
Agent 作为 **第三个分析者**，对比两层输入，做三角差距分析，然后精准追问。

### 工作流程

```
Step 1: ingest_vision      ← 接收战略视角
Step 2: ingest_interview    ← 接收一线访谈（可多次）
Step 3: analyze_gaps        → 三角差距分析 + AI 原生流程重构
Step 4: Agent 精准追问       → 带着分析结果向业务方确认
Step 5: generate_harness    → 生成完整执行手册
Step 6: 发布 & 执行          → Agent 按手册逐阶段落地
```
...
Harness Factory: **two-layer input + triangular analysis**.
...
The Agent acts as the **third analyst** — comparing both layers, performing triangular gap analysis, then asking targeted questions.

### Workflow

```
Step 1: ingest_vision      ← Receive strategic vision
Step 2: ingest_interview    ← Receive frontline interviews (multiple)
Step 3: analyze_gaps        → Triangular gap analysis + AI-native process redesign
Step 4: Agent asks targeted  → Confirm key decisions with stakeholders
        questions
Step 5: generate_harness    → Generate complete execution handbook
Step 6: Publish & Execute   → Agent executes phase by phase
```
...
| `analyze_gaps` | Triangular gap analysis — the core Tool |
...
│       ├── analyze-gaps.ts      # Triangular gap analysis
