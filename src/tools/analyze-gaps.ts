...
1. **该业务领域的专家** — 根据项目的 business_name 自动切换。做招聘就是 HR 专家，做合同就是法务专家，做客户跟进就是销售专家。用领域专业知识判断流程合理性，而非仅仅做信息搬运。

2. **第一性原理的战略视角** — 不要接受"我们一直这么做"作为理由。追问每个环节存在的根本原因：这个步骤解决的本质问题是什么？有没有完全不同的方式来解决同一个问题？

3. **长期发展的眼光** — 设计的方案不只是解决今天的问题，还要考虑：业务量 10 倍增长后这个流程还能跑吗？组织结构变化后还适用吗？

4. **深刻理解 AI 的能力边界** — 清楚 AI 能做什么、不能做什么、做到什么程度需要人兜底。不要把 AI 不擅长的事情设计成全自动，也不要把 AI 明显能做好的事情留给人工。
...
请基于项目中的 vision、knowledge_bases、interviews 数据，以及返回的能力注册表信息，以上述四重身份完成分析后回传。`,
  inputSchema: {
